...
public class ModelServiceImpl implements ModelService {
        ...
        public DetailModelResponse getModelDetail(Long modelId) {
                ...
                String activeModelPolicyName = model.getModelPolicies().stream()
                                .filter(mp -> mp.getStatus() == ModelPolicy.Status.ACTIVE
                                                && (mp.getExpiryDate() == null
                                                                || !mp.getExpiryDate().isBefore(LocalDate.now())))
                                .map(mp -> mp.getWarrantyPolicy().getName())
                                .findFirst() // chỉ lấy một policy (trong trường hợp có nhiều, lấy cái đầu tiên)
                                .orElse(null);
                ...
                // Trả về response có thêm tên ModelPolicy
                return DetailModelResponse.builder()
                                .modelPolicyName(activeModelPolicyName)
